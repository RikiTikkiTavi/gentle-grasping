...
class DataCollection(object):
    ...
    def data_collection(self):
        ...
        # dirname = "data_" + time.strftime("%Y_%m_%d_%H%M%S")
        # data_dir_path = os.path.abspath("../../../data/collected_data") + "/" + dirname
        ...
        for t in range(hz * 4):
            ...
            if t > hz * 3:
                self.allegro.command_joint_position(hand_min)
            # if t == hz * 3.5:  # s_12 (for data augumentation)
            #     save_obs(color_image, depth_image, frame_thumb, frame_middle, 12)
            ros_rate.sleep()
        ...
        # clip and randomize the regrasping pose
        regrasp_x = random_sample(self.cfg.control.regrasp_xy)
        regrasp_y = random_sample(self.cfg.control.regrasp_xy)
        regrasp_z = random_sample(self.cfg.control.regrasp_z)
        if grasp_z + regrasp_z < self.cfg.control.range_z[0]:
            regrasp_z = self.cfg.control.range_z[0] - grasp_z
        elif grasp_z + regrasp_z > self.cfg.control.range_z[1]:
            regrasp_z = self.cfg.control.range_z[1] - grasp_z
        regrasp_rot = random_sample(self.cfg.control.regrasp_rot)
        ...
        # np.save(data_dir_path + "/action1_hand", action1_hand.cpu())
