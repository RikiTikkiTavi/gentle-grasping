...
# https://github.com/facebookresearch/digit-interface/blob/main/digit_interface/digit.py
# --------------------------------------------------------
import logging
import typing
...
class Digit(DigitDefaults):
    ...
    def set_fps(self, fps: int) -> None:
        ...
        This should typically be called after the resolution is set
        as the stream fps defaults to the highest fps
    ...
    def set_intensity_rgb(
        ...
        logger.debug(
            f"{self.serial}:LED intensity set to {intensity} \
                (R: {intensity_r} G: {intensity_g} B: {intensity_b}"
        )
    ...
    def populate(self, serial: str) -> None:
        """
        Find the connected DIGIT based on the serial number and populate device parameters
        into the class
